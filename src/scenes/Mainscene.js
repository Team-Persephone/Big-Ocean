...
//        console.log("friend--->", friend, 'playerFriend--->', playerFriend); 
	...
	update() {
		const scene = this;
		//update the currentPlayer movement
		if (this.scubaDiver) {
			this.scubaDiver.update(this.cursors);

			//update playerFriends movements
			let x;
			let y;
			x = this.scubaDiver.x;
			y = this.scubaDiver.y;
			...
			if (
				this.scubaDiver.oldPosition &&
				(x !== this.scubaDiver.oldPosition.x ||
					y !== this.scubaDiver.oldPosition.y)
			) {
